<script lang="ts">
	...
	const TYPES = ['text', 'number', 'checkbox', 'date', 'select', 'multiSelect'] as const;
	...
	function emit() {
		onData(JSON.stringify({ columns, rows, view, groupBy }));
	}

	// Sync from node attribute (load, undo/redo) — skip when it's our own echo.
	$effect(() => {
		let parsed: { columns?: DBColumn[]; rows?: DBRow[]; view?: 'table' | 'kanban'; groupBy?: string | null };
		try {
			parsed = JSON.parse(data);
		} catch {
			parsed = {};
		}
		const d = parsed as typeof parsed;
		const sig = JSON.stringify({ columns, rows, view, groupBy });
	...
	function compare(a: string | boolean | string[], b: string | boolean | string[], col: DBColumn): number {
		...
		if (col.type === 'number') return (parseFloat(av) || 0) - (parseFloat(bv) || 0);
		if (col.type === 'date') return (Date.parse(av) || 0) - (Date.parse(bv) || 0);
...
<div class="db" data-database>
	<div class="db-toolbar">
		<div class="db-view-switch" role="group" aria-label="View">
			<button class="db-view-btn" class:active={view === 'table'} onclick={() => setView('table')}>Table</button>
			<button class="db-view-btn" class:active={view === 'kanban'} onclick={() => setView('kanban')}>Kanban</button>
		</div>
		{#if view === 'kanban'}
						...
						{#each g.rows as row (row.id)}
							<div class="db-kb-card" role="listitem" draggable="true" ondragstart={(e) => e.dataTransfer?.setData('text/row', row.id)}>
								{#each columns as col (col.id)}
									...
									{:else if col.type === 'date' && col.id !== groupBy}
										<span class="db-kb-date">{String(cellValue(row, col)).slice(0, 10)}</span>
									{:else if col.id !== groupBy}
										<span class="db-kb-text">{String(cellValue(row, col))}</span>
									{/if}
								{/each}
							</div>
						{/each}
